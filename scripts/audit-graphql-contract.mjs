...
import {
	...
	generatedTrees,
} from './lib/module-resolution.mjs';
...
function walk(dir, extensions, out = []) {
	for (const entry of readdirSync(dir).sort()) {
		// Never source, anywhere, in any repository: excluding these two is not a
		// judgement call about this tree's layout. Build OUTPUT is not excluded by
		// name — see `generatedTrees`, which asks the build where it writes, because
		// a name list refused `src/lib/build/absent-renderer-module.ts` the first
		// time it ran.
...
async function main(argv) {
	...
	// --- what counts as executable source, asked rather than declared ---------
	//
	// This used to be a literal list in the pin, and it omitted `.tsx` and `.jsx`.
	// Vite resolves and builds both; the review made an entry import a `.tsx` module
	// that sent a dynamically assembled invalid document through the production
	// transport and the gate exited 0/PASS. The set now comes from the resolver that
	// runs — Vite's own `resolve.extensions` plus whatever its loader plugins claim —
	// so a Vite upgrade that adds a suffix widens this walk instead of leaving a hole.
	const resolver = await createViteResolver({ root: ROOT, configFile: viteConfigIn(ROOT) });
	let closure;
	let extensions;
	let files;
	try {
		if (Array.isArray(roots?.extensions)) {
			fail([
				`${PROVENANCE} still declares document_roots.extensions as a flat list. That list is ` +
					'what omitted .tsx/.jsx while Vite built them; the set is now derived from the ' +
					'resolver, and only the deltas are declared (additional_extensions / ' +
					'excluded_extensions). Remove it rather than leaving two answers in the tree.',
			]);
		}
		extensions = executableExtensions(resolver, {
			additional: roots?.additional_extensions ?? [],
			excluded: roots?.excluded_extensions ?? [],
		});
		if (!extensions.length) {
			fail([
				'the resolver reported no executable source extensions, so this walk would read ' +
		...
		// --- documents ---------------------------------------------------------
		//
		// A document root that contains a directory the build WRITES would put
		// generated bytes in front of this reader — compiled copies of every module,
		// each one a document nobody wrote. Asked of the build rather than guessed at.
		findings.push(...generatedTrees(resolver, ROOT, roots.paths));
		files = roots.paths.flatMap((root) => walk(path.join(ROOT, root), extensions));
		...
		closure = await resolveClosure(
			ROOT,
			[...files, ...entryPoints],
			resolver,
			isScriptIn(extensions)
		);
	} finally {
	...
	// The graph's own uncertainty is a gate finding, not an internal note. A budget
	// or a fixpoint that stopped early has not said "no transport here" — it has
	// said it stopped, and the difference is the whole of the review's second
	// transport finding.
	for (const finding of channels.findings) {
		if (isUpstream(finding.file)) continue;
		findings.push(`${finding.file}:${finding.line} ${finding.name} — ${finding.reason}`);
	}
	...
	for (const tree of upstreamTrees) {
		...
		// AN ORPHAN OUTSIDE THE BOUNDARY IS A FINDING UNLESS IT IS DECLARED.
		//
		// This used to be a PASS note — a count printed beside a green result. Review
		// was right that a note is not a control: it reports the same thing whether
		// the situation is the known, upstream-owned one or a new latent path someone
		// added this morning. The declared set below is asserted EXACTLY, in both
		// directions: an undeclared orphan fails, and a declared one that no longer
		// reaches anything fails too, because a disclosure that has stopped describing
		// something is a pin that has stopped asserting.
		const declaredOrphans = tree.outside_boundary_orphans?.files ?? {};
		const orphanReaches = [];
		...
		for (const file of orphanReaches) {
			if (!declaredOrphans[file]) {
				findings.push(
					`${file} sits outside ${tree.path}, reaches a disclosed vendored GraphQL document, ` +
						'and is not declared in contracts/lesser/provenance.json. Nothing executes it ' +
						'today — it is unreachable from every build entry — but it is a latent path into ' +
						'documents this gate has excluded, and a latent path nobody declared is one ' +
						'nobody owns. Declare it with the upstream owner and the channel that would ' +
						'remove it, or remove it through that channel.'
				);
			}
		}
		for (const file of Object.keys(declaredOrphans)) {
			if (!orphanReaches.includes(file)) {
				findings.push(
					`${file} is declared as an outside-boundary orphan in ` +
						'contracts/lesser/provenance.json but is no longer one — it either went away or ' +
						'became reachable. Either way the declaration has stopped describing the tree, ' +
						'so it must be removed or re-stated deliberately.'
				);
			}
		}
		if (orphanReaches.length) {
			boundaryNotes.push(
				`${orphanReaches.length} declared outside-boundary orphan(s) in ${tree.path}'s ` +
					`disclosure — unreachable from every entry, upstream-owned, and asserted exactly ` +
					`so a new one fails rather than joining a count: ${orphanReaches.join(', ')}`
			);
		}
		for (const file of Object.keys(declared)) {
	...
	process.stdout.write(
		`graphql-contract: executable source is ${extensions.join(' ')} — derived from Vite's own ` +
			`resolve.extensions plus its loader plugins, not declared here\n`
	);
	process.stdout.write(
