...
"$TITLE")` consumes data, `$(bash -c "$TITLE")` does not.
  ...
  Reading the segment head alone left three shapes open, each carrying the same value to
  the same interpreter by a different spelling: a launcher wrapper in front of it (`env
bash -c "$TITLE"`, `command bash -c "$TITLE"`, and wrapper chains), the program handed
  to the interpreter on standard input rather than after a flag (`bash <<< "$TITLE"`,
  `bash < <(…)`), and `source`/`.` applied to a process substitution, whose interior
  read on its own is a data use while `source` executes what it prints. Resolution now
  steps over wrapper prefixes to the executed command; stdin-fed program text is read as
  program text; and `source`/`.` treat every operand as the program. Over-blocking is
  bounded by the same data/program line as before: `env VAR=value ./script.sh`, `env
printf '%s' "$TITLE"`, and `bash pinned.sh <<< "$TITLE"` — where the operand names the
  script that owns stdin — all stay accepted.
