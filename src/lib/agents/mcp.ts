 ...
 *
 * ONE READER NOW, AND THE RULE DID NOT CHANGE. The detail panel
 * (`AgentMcpPanel`) shows the whole bundle and probes it. There used to be a
 * second one: the grantee's "shared with you" list reduced the bundle to the
 * single line a grantee needs — the endpoint they connect to — through a
 * `sharedMcpAccess` classifier kept here rather than in the component, because
 * `node --test` can drive a module and cannot mount a component.
 * equaltoai/contentus#119 removed that reader. The list links to the agent page,
 * where the whole bundle is already rendered, instead of issuing one GraphQL
 * read per row to restate a URL that link leads to; the classifier went with it,
 * because an export with no caller is a rule nobody is following. What survives
 * is the part that mattered, and it is unchanged: every URL here is read from
 * `Agent.mcpAccess`, and no part of one is built.
 *
